...
func RunProducer(ctx context.Context) {
	...
	stream, err := js.CreateStream(ctx, defaultJetStreamConfig())
	if err != nil {
	...
	for i := range 10 {
		...
		ack, err := js.Publish(ctx, Subject, []byte("hello:"+strIdx+"-"+strconv.Itoa(i)))
		if err != nil {
			logger.Error("Failed to publish message", slog.Any("error", err))
			continue
		}
		logger.Info("Published message", slog.Any("ack", ack))
	}
